...
// React Native's fetch is fully native and non-blocking — no bridge plumbing needed.
export async function doFetch(url, options = {}) {
  return fetch(url, options);
}

// RN has no atob(); small base64 decoder for the Saavn cipher below
function atob(input) {
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
  const str = input.replace(/=+$/, '');
  let out = '';
  for (let i = 0; i < str.length; i += 4) {
    const n = ((chars.indexOf(str[i]) & 0xff) << 18)
      | ((chars.indexOf(str[i + 1]) & 0xff) << 12)
      | ((str[i + 2] !== undefined ? chars.indexOf(str[i + 2]) : 0) << 6)
      | (str[i + 3] !== undefined ? chars.indexOf(str[i + 3]) : 0);
    out += String.fromCharCode((n >> 16) & 0xff);
    if (str[i + 2] !== undefined && str[i + 2] !== '=') out += String.fromCharCode((n >> 8) & 0xff);
    if (str[i + 3] !== undefined && str[i + 3] !== '=') out += String.fromCharCode(n & 0xff);
  }
  return out;
}

// RN's Hermes has no TextDecoder either — manual UTF-8 byte decoder
...
function decryptDES_ECB(base64Cipher, keyStr = '38346591') {
  ...
  try {
    ...
    // RN's Hermes engine has no TextDecoder — decode UTF-8 manually
    const url = utf8Decode(finalBytes);
